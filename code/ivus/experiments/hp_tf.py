...
space = {
    ...
    'tf_length': hp.quniform(
        'tf_length', MIN_TF_LENGTH, MAX_TF_LENGTH, STEP_TF_LENGTH),
}

sequence_reader = SequenceReader(CNN_PREDICTIONS, FOLDS)
csv_headers = [
    'model_id', 'fold', 'beta_1', 'lr', 'recurrent_dropout', 'lstm_units',
    'tf_length', 'epoch', 'loss', 'acc', 'f1', 'val_loss', 'val_acc', 'val_f1',
    'time']
...
def cross_validation(params, sequence_reader=sequence_reader):
    ...
    for dataset in sequence_reader.get_folded_dataset(
        int(params['tf_length']), train_rnn=True, val=True
    ):
...
for rnn_model in top_5_best:
    ...
    for dataset in sequence_reader.get_folded_dataset(
        int(rnn_model['params']['tf_length']), train_rnn=True, val=True
    ):
        history = model.train(
